...
- **⚠️ 당일 데이터 사용 필수:** 네이버 증권 투자자별 순매매 상위 페이지는 **좌(전날)·우(당일)** 두 컬럼을 나란히 표시한다. `re.findall` 결과는 `[전날1위,...,전날20위, 당일1위,...,당일20위]` 순으로 반환되므로 `all_items[len(all_items)//2:]` (후반부)가 당일 데이터다. `investor` 명령 결과에서도 날짜 컬럼을 확인하고 오늘 날짜 행만 분석에 반영한다.
