...
> **🚨 [2026-08-11 버그 수정] 반드시 `[-(len//2):]`를 쓸 것 — `[len//2:]`는 틀렸다.**
>
> 종전 표기 `all_items[len(all_items)//2:]`는 **전체 매치 수가 짝수일 때만 정확**하다.
> 종목명 파싱 실패 등으로 매치 수가 **홀수**가 되면 `len//2`가 내림되어 **전날 데이터 1행이 앞에 딸려 들어온다.**
>
> | 매치 수 | `[len//2:]` | `[-(len//2):]` |
> | ---: | :--- | :--- |
> | 40 (짝수) | 20건 — 정확 | 20건 — 정확 |
> | **39 (홀수)** | **20건 — 전날 1행 오염 ❌** | **19건 — 정확 ✅** |
>
> **실제 사고:** 2026-08-11 기관 순매수 상위(KOSPI) 매치 39건에서 **한화오션이 1위로 잘못 산출**됐다.
> 표시된 `+14,253백만원`은 **8/10(전날)** 수치였고, 당일 한화오션은 **기관 -375,030주 순매도**였다.
>
> **검증 습관:** 추출 후 순매수 금액이 **내림차순인지 확인한다.** 1위가 2위보다 작으면 전날 행이 섞인 것이다.
> 의심되면 `python scripts/naver_finance.py investor [종목코드]`로 당일 수치를 직접 대조한다.
